...
public class OpExample { // 예제 코드 작성용 클래스 
	...
	public void ex4() {
		...
		System.out.println("temp는 짝수인가? " + (temp % 2 == 0 )); //f
		System.out.println("temp는 짝수인가? " + (temp % 2 != 1 )); //f
		
		System.out.println("temp는 홀수인가? " + (temp % 2 == 1 )); //t
		System.out.println("temp는 홀수인가? " + (temp % 2 != 0 )); //t
	...
	public void ex5() {
		// 논리 연산자 : &&(AND), ||(OR)
		// &&(AND) 연산자 : 둘다 true 일때만 true, 나머진 false
		...
		System.out.println(1 <= b && b <= 10); // b는 10 이하인가?
		
		System.out.println("--------------------------------------------------");
		
		// || (OR) 연산자 : 둘다 false 이면 false, 나머지는 true (AND의 반대)
		// ~또는, ~거나, ~이거나
